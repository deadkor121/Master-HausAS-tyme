...
## Штатный процесс миграций

Сейчас база уже приведена к baseline, поэтому дальше используйте обычный deploy-путь Prisma без ручного `db execute`.

### Для локальной/CI среды
...
### Для новой базы один раз

Если вы поднимаете полностью пустую БД и хотите применять историю миграций с нуля, сначала выполните обычный deploy. Если база уже была создана раньше без истории миграций, один раз зафиксируйте baseline через `prisma migrate resolve`, после чего снова используйте только deploy.

### В этом проекте baseline уже сделан

Для текущей базы уже отмечены применёнными миграции:

- `20260708095405_init`
- `20260718141500_worker_advances`
- `20260718144000_worker_profile_fields`
- `20260804124500_geolocation_features`

После этого все новые миграции должны идти через `npm run prisma:migrate:deploy --workspace apps/api`.
...
- геолокация рабочего места, выход из зоны и фотоотчеты
