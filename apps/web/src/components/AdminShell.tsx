...
function NavItem({ to, label }: { to: string; label: string }) {
  ...
  return (
    <Link
      to={to}
      className={`rounded-2xl px-4 py-3 text-sm transition ${active ? 'bg-cyan-400 text-slate-950 shadow-lg shadow-cyan-500/20' : 'bg-white/5 text-slate-300 hover:bg-white/10 hover:text-white'}`}
    >
      {label}
    </Link>
  );
      ...
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col lg:flex-row">
        <aside className="border-b border-white/10 bg-slate-950/70 p-4 backdrop-blur lg:min-h-screen lg:w-80 lg:border-b-0 lg:border-r lg:p-6">
          ...
          <div className="mt-6 space-y-2">
            ...
            <NavItem to="/dashboard" label="Дашборд" />
            <NavItem to="/orders" label="Заказы" />
            <NavItem to="/workers" label="Работники" />
            <NavItem to="/worker-attendance" label="Учёт времени" />
            <NavItem to="/accounting" label="Бухгалтерия" />
          </div>
