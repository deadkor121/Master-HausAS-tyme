...
export default function WorkerAttendancePage() {
        ...
        <div className="space-y-6">
          <form onSubmit={submit} className="grid gap-4 rounded-[2rem] border border-white/10 bg-white/5 p-5 md:p-6">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-400">{isWorkerOnly ? 'Личная страница работника для постановки даты и времени работы.' : 'Админский режим для управления табелями сотрудников.'}</p>
            </div>
  ...
  return isWorkerOnly ? (
    <WorkerShell title="Мои смены" description="Личная страница работника: ставь дату и время работы, смотри календарь и заработок за месяц.">{content}</WorkerShell>
  ) : (
